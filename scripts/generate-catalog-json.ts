...
# Optional local overrides (used for offline/cached runs; safe no-ops otherwise)
LOCAL_CATALOG_PATH = os.environ.get("GMTS_LOCAL_CATALOG", "")
LOCAL_LOGO_PATH = os.environ.get("GMTS_LOCAL_LOGO", "")
LOCAL_IMAGE_MAP = {}
_local_map_env = os.environ.get("GMTS_LOCAL_IMAGE_MAP", "")
if _local_map_env:
    for pair in _local_map_env.split("||"):
        if "->" in pair:
            k, v = pair.split("->", 1)
            LOCAL_IMAGE_MAP[k] = v
...
def fetch_image(url):
    ...
    if url in LOCAL_IMAGE_MAP and os.path.exists(LOCAL_IMAGE_MAP[url]):
        try:
            return Image.open(LOCAL_IMAGE_MAP[url]).convert("RGBA")
        except Exception:
            pass
    try:
...
def fetch_catalog():
    ...
    if LOCAL_CATALOG_PATH and os.path.exists(LOCAL_CATALOG_PATH):
...
def parse_estimate(pdf_path):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for i, w in enumerate(words):
            ...
            if w["text"].upper() == "DATE" and not date_found:
                if i + 1 < len(words) and re.match(r"^\d{1,2}/\d{1,2}/\d{2,4}$", words[i + 1]["text"]):
        ...
        if address_x0 is not None and ship_x0 is not None:
            ...
            # Collect words in the address column, strictly below the
            # ADDRESS/SHIP TO header row (using the header's own top
            # position, not a fixed pixel threshold, avoids picking up
            # the "Estimate" document title printed above it).
            addr_words = [
                w for w in words
                if w["x0"] >= address_x0 and w["x1"] <= right_edge + 20
                and w["top"] > address_top + 1
            ]
...
# description (everything after). Mixed-case leading words (e.g. a
# service line like "Inside Delivery") won't match the SKU pattern and
# are correctly excluded as non-product lines.
TAIL_PATTERN = re.compile(
    r"^(.*?)\s+(\d+(?:\.\d+)?)\s+[\d,]+\.\d{2}\s+[\d,]+\.\d{2}T?\s*$"
)
SKU_HEAD_PATTERN = re.compile(
    r"^([A-Z0-9]+(?:[\-\/][A-Z0-9]+)*(?:\s[A-Z0-9]+(?:[\-\/][A-Z0-9]+)*)?)\s+(.+)$"
)
...
def _parse_line_item(line):
    ...
    tail_m = TAIL_PATTERN.match(line)
    if not tail_m:
        return None

    head = tail_m.group(1).strip()
    try:
        quantity = int(float(tail_m.group(2)))
    except ValueError:
        quantity = 1

    head_m = SKU_HEAD_PATTERN.match(head)
    if not head_m:
        # No SKU-like leading token (e.g. a service line like
        # "Inside Delivery, Assembly and Trash Removal") — skip it.
        return None

    sku_raw = head_m.group(1).strip()
    description = head_m.group(2).strip()
