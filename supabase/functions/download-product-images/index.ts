...
async function fetchOne(rel: string): Promise<{ rel: string; bytes?: Uint8Array; error?: string; status?: number }> {
  try {
    ...
    if (!res.ok) return { rel, error: `HTTP ${res.status}`, status: res.status };
    const buf = new Uint8Array(await res.arrayBuffer());
    return { rel, bytes: buf };
  } catch (e) {
    return { rel, error: String(e) };
  }
}
...
Deno.serve(async (req) => {
  ...
  const manifest: Array<{ rel: string; zip_name: string; bytes: number; ok: boolean; error?: string }> = [];
  ...
  async function worker() {
    while (idx < IMAGE_PATHS.length) {
      const i = idx++;
      const rel = IMAGE_PATHS[i];
      const r = await fetchOne(rel);
      const zipName = rel.replaceAll("/", "__");
      if (r.bytes) {
        files[zipName] = r.bytes;
        manifest.push({ rel, zip_name: zipName, bytes: r.bytes.length, ok: true });
      } else {
        manifest.push({ rel, zip_name: zipName, bytes: 0, ok: false, error: r.error });
      }
    }
  }
  ...
  files["MANIFEST.json"] = strToU8(JSON.stringify(summary, null, 2));
  ...
  return new Response(zipped, {
    ...
    headers: {
      ...
      "Content-Disposition": 'attachment; filename="gmts-product-images.zip"',
      "X-Image-Count": String(summary.succeeded),
      "X-Image-Failed": String(summary.failed),
    },
  });
});
